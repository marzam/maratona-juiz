...
//INSERT INTO login (name, password, score, type, username) values ('Marcelo', '12345', '0', '1', 'mzamith')
  ...
  $sql = 'SELECT submission.*,problem.file as file_prob,problem.inputHPC,problem.input, problem.output, problem.stdout,problem.time FROM submission INNER JOIN problem ON problem.id = submission.problem_id WHERE  submission.answer = "pending" ORDER BY submission.id ASC LIMIT 1;';
  $result = execQuery($sql);
