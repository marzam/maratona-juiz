...
def exec(binfile, srcpath, param, mtimeout):
    ...
    if len(stdout) > 0:
        str_stdout = '\t\t' + stdout.decode('utf-8', 'ignore').replace('\n', '\n\t\t')
        print('\tOUT:')
        print(str_stdout)
        a_stdout = stdout.decode('utf-8', 'ignore').replace('\n', '')

    if len(stderr) > 0:
        str_sterr = '\t\t' + stderr.decode('utf-8', 'ignore').replace('\n', '\n\t\t')
        print('\tERR:')
        print(str_sterr)
...
def exec_job(opt, job):
    ...
    try:
        ...
        data = {'id': job['id'], 'nameLogin': opt.name, 'namePassed': opt.pw, 'answer': 'submitted file corrupted', 'info': ''}
    ...
    binfile, answer, info = compile_make(srcpath)
    if answer != '':
        data = {'id': job['id'], 'nameLogin': opt.name, 'namePassed': opt.pw, 'answer': answer, 'info':info}
        response = opt.session.post(url=opt.url + 'setsubtime.php', data=data)
        if (opt.verbose): print(response.text)
        print(info)
        sys.exit(0)
    ...
    if job['stdout'].find('file') >= 0:
        ...
        template = tmpdir + '/template.bin'
        answer = equal_bin(template, file_created)
    ...
    if answer != '':
        data = {'id': job['id'], 'nameLogin': opt.name, 'namePassed': opt.pw, 'answer': 'wrong answer', 'info': ''}
        if (opt.verbose): print(data)
        response = opt.session.post(url=opt.url + 'setsubtime.php', data=data)
        if (opt.verbose): print(response.text)
    ...
    if subtime < 0.0:
        data = {'id': job['id'], 'nameLogin': opt.name, 'namePassed': opt.pw, 'answer': 'time out', 'info': ''}
        response = opt.session.post(url=opt.url + 'setsubtime.php', data=data)
        if (opt.verbose): print(response.text)
        return
